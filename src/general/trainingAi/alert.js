    ...
    const chatSession = model.startChat({
      ...
      history: [
        ...
        },
        {
          role: "user",
          parts: [
            {text: "me de uma alerta"},
          ],
        },
        {
          role: "model",
          parts: [
            {text: "The user wants a short, non-annoying daily alert about malaria prevention. I should generate a JSON object following the provided schema, which only requires the `recommendation` field in this case, as `is_right` doesn't apply to a general alert. The alert should be simple and actionable.\n\n1.  **Goal:** Daily malaria prevention alert.\n2.  **Constraint:** Lightweight and not annoying.\n3.  **Schema:** JSON with `recommendation` field.\n4.  **Content Idea:** Focus on a simple, common prevention method. Checking for standing water is a good daily habit.\n5.  **Format:** JSON object.\n\nPlan:\nCreate a JSON object with the key \"recommendation\" and a value containing a short, actionable message about checking for standing water."},
            {text: "{\n  \"is_right\": false,\n  \"recommendation\": \"Alerta Malária: Verifique hoje se há água parada perto de casa. Um minuto pode prevenir picadas! Elimine recipientes que acumulam água.\"\n}"},
          ],
        },
      ],
    });
  
    const result = await chatSession.sendMessage("me de uma alerta");
